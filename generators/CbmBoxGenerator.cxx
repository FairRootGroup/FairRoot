// -------------------------------------------------------------------------
// -----            CbmBoxGenerator source file                        -----
// -----          Created 09/09/04  by Yu.Kharlov
// -------------------------------------------------------------------------

/* $Id: CbmBoxGenerator.cxx,v 1.4 2006/07/18 09:28:06 prokudin Exp $ */

/* History of cvs commits:
 *
 * $Log: CbmBoxGenerator.cxx,v $
...
// ------------------------------------------------------------------------
CbmBoxGenerator::CbmBoxGenerator() :
...
// ------------------------------------------------------------------------
CbmBoxGenerator::CbmBoxGenerator(Int_t pdgid, Int_t mult) :
...
// ------------------------------------------------------------------------
void CbmBoxGenerator::Init()
{
  // Initialize generator

  if (fPhiMax-fPhiMin>360)
    Fatal("Init()","CbmBoxGenerator: phi range is too wide: %f<phi<%f",
	  fPhiMin,fPhiMax);
  if (fPRangeIsSet && fPtRangeIsSet)
    Fatal("Init()","CbmBoxGenerator: Cannot set P and Pt ranges simultaneously");
  if (fPRangeIsSet && fYRangeIsSet)
    Fatal("Init()","CbmBoxGenerator: Cannot set P and Y ranges simultaneously");
  if ( (fThetaRangeIsSet && fYRangeIsSet) ||
       (fThetaRangeIsSet && fEtaRangeIsSet) ||
       (fYRangeIsSet     && fEtaRangeIsSet) )
    Fatal("Init()","CbmBoxGenerator: Cannot set Y, Theta or Eta ranges simultaneously");
  if (fPointVtxIsSet && fBoxVtxIsSet)
    Fatal("Init()","CbmBoxGenerator: Cannot set point and box vertices simultaneously");

  // Check for particle type
  TDatabasePDG* pdgBase = TDatabasePDG::Instance();
  TParticlePDG *particle = pdgBase->GetParticle(fPDGType);
  if (! particle) Fatal("CbmBoxGenerator","PDG code %d not defined.",fPDGType);
  fPDGMass = particle->Mass();
}

// ------------------------------------------------------------------------
Bool_t CbmBoxGenerator::ReadEvent(CbmPrimaryGenerator* primGen)
{
