...
ClassImp(CbmRuntimeDb)

CbmRuntimeDb* CbmRuntimeDb::gRtdb=0;

CbmRuntimeDb* CbmRuntimeDb::instance(void) {
  // Singleton instance
  if (gRtdb==0) gRtdb=new CbmRuntimeDb;
  return gRtdb;
}

CbmRuntimeDb::CbmRuntimeDb(void) {
...
void CbmRuntimeDb::addContFactory(CbmContFact* fact) {
...
Bool_t CbmRuntimeDb::addParamContext(const char* context) {
...
void CbmRuntimeDb::printParamContexts() {
  // Prints the context of all parameter containers, which can be created by
  // the container factories
  TIter next(&contFactories);
  CbmContFact* fact;
  while((fact=(CbmContFact*)next())) fact->print();
}

Bool_t CbmRuntimeDb::addContainer(CbmParSet* container) {
  ...
  if (!containerList->FindObject(name)) {
    ...
    CbmRtdbRun* run;
    CbmParVersion* vers;
    while ((run=(CbmRtdbRun*)next())) {
      if (!run->getParVersion(name)) {
        vers=new CbmParVersion(name);
        run->addParVersion(vers);
      }
    }      
    return kTRUE;
  }
  Warning("addContainer(CbmParSet*)","Container %s already exists!",name);
  return kFALSE;
}
...
CbmParSet* CbmRuntimeDb::findContainer(const char* name) {
  ...
  return (CbmParSet*)(containerList->FindObject(name));
}

void CbmRuntimeDb::removeContainer(Text_t* name) {
...
void CbmRuntimeDb::removeAllContainers(void) {
  // removes all containers from the list and deletes them
  containerList->Delete();
}

CbmRtdbRun* CbmRuntimeDb::addRun(Int_t runId,Int_t refId) {
  ...
  run=getRun(runId);
  if (!run) {
    run=new CbmRtdbRun(runId,refId);
    TIter next(containerList);
    CbmParSet* cont;
    CbmParVersion* vers;
    while ((cont=(CbmParSet*)next())) {
      vers=new CbmParVersion(((char*)cont->GetName()));
      run->addParVersion(vers);
    }    
    runs->Add(run);
    currentRun=run;
...
CbmRtdbRun* CbmRuntimeDb::getRun(Int_t id) {
  // returns a pointer to the run called by the run id
  char name[255];
  sprintf(name,"%i",id);
  return (CbmRtdbRun*)(runs->FindObject(name));
}

CbmRtdbRun* CbmRuntimeDb::getRun(Text_t* name) {
  // returns a pointer to the run called by name
  return (CbmRtdbRun*)(runs->FindObject(name));
}

void CbmRuntimeDb::removeRun(Text_t* name) {
...
void CbmRuntimeDb::clearRunList() {
  runs->Delete();
}

void CbmRuntimeDb::writeVersions() {
...
Bool_t CbmRuntimeDb::writeContainers() {
  ...
  if (currentRun) {
    ...
    if (strlen(refRunName)>0) {
      refRun=(CbmRtdbRun*)(runs->FindObject(refRunName));
    }
    while ((cont=(CbmParSet*)next())) {
...
Int_t CbmRuntimeDb::findOutputVersion(CbmParSet* cont) {
...
Bool_t CbmRuntimeDb::writeContainer(CbmParSet* cont, CbmRtdbRun* run, CbmRtdbRun* refRun) {
  ...
  CbmParVersion* vers=run->getParVersion(c);
  Bool_t rc=kTRUE;
  Int_t cv=0;
  ...
  if (refRun) {
    CbmParVersion* refVers=refRun->getParVersion(c);
    if (refVers) {
...
Bool_t CbmRuntimeDb::initContainers(Int_t runId,Int_t refId,
...
Bool_t CbmRuntimeDb::readAll() {
  ...
  Bool_t rc=kTRUE;
  TIter next(runs);
  while ((currentRun=(CbmRtdbRun*)next())!=0) {
...
Bool_t CbmRuntimeDb::initContainers(void) {
  // private function 
  ...
  if (len<1) {
    ...
    CbmRtdbRun* refRun=getRun(refRunName);
...
void CbmRuntimeDb::setContainersStatic(Bool_t flag) {
...
Bool_t CbmRuntimeDb::setInputVersion(Int_t run,Text_t* container,
  ...
  CbmRtdbRun* r=getRun(run);
  if (r) {
    CbmParVersion* v=r->getParVersion(container);
    if (v) {
...
Bool_t CbmRuntimeDb::setRootOutputVersion(Int_t run,Text_t* container,
                               Int_t version) {
...
void CbmRuntimeDb::resetInputVersions() {
...
void CbmRuntimeDb::resetOutputVersions() {
...
void CbmRuntimeDb::resetAllVersions() {
...
Bool_t CbmRuntimeDb::setFirstInput(CbmParIo* inp1) {
  ...
  else Error("setFirstInput(CbmParIo*)","no connection to input");
  return kFALSE;
}

Bool_t CbmRuntimeDb::setSecondInput(CbmParIo* inp2) {
  ...
  else Error("setSecondInput(CbmParIo*)","no connection to input");
  return kFALSE;
}

Bool_t CbmRuntimeDb::setOutput(CbmParIo* op) {
  // sets the output pointer
  output=op;
  if (output->check()==kTRUE) {
    resetOutputVersions();
    if (strcmp(output->IsA()->GetName(),"CbmParRootFileIo")==0)
        isRootFileOutput=kTRUE;
    return kTRUE;
  }
  else Error("setOutput(CbmParIo*)","no connection to output");
  return kFALSE;
}

CbmParIo* CbmRuntimeDb::getFirstInput() {
  // return a pointer to the first input
  if (firstInput) firstInput->cd();
  return firstInput;
}

CbmParIo* CbmRuntimeDb::getSecondInput() {
  // return a pointer to the second input
  if (secondInput) secondInput->cd();
  return secondInput;
}

CbmParIo* CbmRuntimeDb::getOutput() {
  // return a pointer to the output
  if (output) output->cd();
  return output;
}

void CbmRuntimeDb::closeFirstInput() {
  if (firstInput) {
    firstInput->cd();
    firstInput->close();
    firstInput=0;
...
void CbmRuntimeDb::closeSecondInput() {
  if (secondInput) {
    secondInput->cd();
    secondInput->close();
    secondInput=0;
...
void CbmRuntimeDb::activateParIo(CbmParIo* io) {
  // activates the detector I/O

    const char* ioName=io->IsA()->GetName();
  CbmDetParIo* po=io->getDetParIo("CbmGenericParIo");
  if (!po) {
    if (strcmp(ioName,"CbmParRootFileIo")==0) {
      CbmDetParRootFileIo* pn=
          new CbmGenericParRootFileIo(((CbmParRootFileIo*)io)->getParRootFile());
      io->setDetParIo(pn);
    } else if (strcmp(ioName,"CbmParAsciiFileIo")==0) {
      CbmDetParAsciiFileIo* pn=
          new CbmGenericParAsciiFileIo(((CbmParAsciiFileIo*)io)->getFile());
      io->setDetParIo(pn);
    }
  }
  TIter next(&contFactories);
