...
class CbmParamObj : public TNamed {
protected:
...
public:
  CbmParamObj();
  CbmParamObj(const Text_t*,const Text_t*,const char,const Int_t);
  ~CbmParamObj() {}
  ...
  void print();  
  ClassDef(CbmParamObj,0) // Class for parameter object (name + string-value)
};

class CbmParamBinObj : public TNamed {
private:
  CbmParamBinObj(const CbmParamBinObj &P);
  CbmParamBinObj& operator= (const  CbmParamBinObj&) {return *this;}
...
public:
  ...
  CbmParamBinObj(const Text_t*,const UChar_t*,const Int_t);
  CbmParamBinObj(const Text_t*,const Short_t*,const Int_t);
  CbmParamBinObj(const Text_t*,const Int_t*,const Int_t);
  CbmParamBinObj(const Text_t*,const Float_t*,const Int_t);
  CbmParamBinObj(const Text_t*,const Double_t*,const Int_t);
  ~CbmParamBinObj();
  ...
  Int_t getNumParams();
  void print();  
  ClassDef(CbmParamBinObj,0) // Class for binary parameter object (name + binary array)
};

class CbmParamList : public TObject {
private:
  CbmParamList(const CbmParamList &P);
  CbmParamList& operator= (const  CbmParamList&) {return *this;}
...
public:
  CbmParamList();
  ~CbmParamList();
  void add(CbmParamObj&);
  void addBinary(CbmParamBinObj&);
  ...
  void print();
  CbmParamObj* find(const Text_t* name) {
    return (CbmParamObj*)paramList->FindObject(name);
  }
  CbmParamBinObj* findBinary(const Text_t* name) {
    return (CbmParamBinObj*)paramBinList->FindObject(name);
  }
  TList* getList() { return paramList; }
  TList* getBinaryList() { return paramBinList; }
  ClassDef(CbmParamList,0) // Class for lists of parameters (of type CbmParamObj and CbmParamBinObj)
};
