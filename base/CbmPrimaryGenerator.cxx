// -------------------------------------------------------------------------
// -----                CbmPrimaryGenerator source file                -----
...
CbmPrimaryGenerator::CbmPrimaryGenerator(const char* name, const char* title) 
  :TNamed(name,title),
   fBeamX0(0),
   fBeamY0(0),
   fBeamSigmaX(0),
...
Bool_t CbmPrimaryGenerator::GenerateEvent(CbmGenericStack* pStack) {
  ...
  while( (obj = fListIter->Next()) ) {
    gen = dynamic_cast<CbmGenerator*> (obj);
...
void CbmPrimaryGenerator::AddTrack(Int_t pdgid, Double_t px, Double_t py,
  ...
  if ( ! pdgBase ) Fatal("CbmPrimaryGenerator", 
			 "No TDatabasePDG instantiated");
  TParticlePDG* pdgPart = pdgBase->GetParticle(pdgid);
  if ( ! pdgPart ) {
    cout << "-E CbmPrimaryGenerator: PDG code " << pdgid 
...
void CbmPrimaryGenerator::SetBeam(Double_t x0, Double_t y0,
...
void CbmPrimaryGenerator::SetTarget(Double_t z, Double_t dz) {
...
void CbmPrimaryGenerator::SetMultTarget(Int_t nroftargets, Double_t *targetpos, Double_t dz) {
...
void CbmPrimaryGenerator::MakeVertex() {
  Double_t vx = fBeamX0;
  Double_t vy = fBeamY0;
  Double_t vz;
  if (1 == fNrTargets) {
...
void CbmPrimaryGenerator::SetEventTimeInterval(Double_t min, Double_t max)
{
   fEventTimeMin=min;
   fEventTimeMax=max;
...
void  CbmPrimaryGenerator::SetEventMeanTime(Double_t mean)
{
   fEventMeanTime =mean;
   TString form="(1/";
   form+= mean;
