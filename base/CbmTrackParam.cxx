...
CbmTrackParam::CbmTrackParam(Double_t x, Double_t y, Double_t z,
...
// -------------------------------------------------------------------------
// -----   Copy Constructor             -----------------------------------
CbmTrackParam::CbmTrackParam(const CbmTrackParam& param) 
: fX(param.GetX()),
  fY(param.GetY()),
  fZ(param.GetZ()),
  fTx(param.GetTx()),
...
void CbmTrackParam::Momentum(TVector3& mom) const {
...
void CbmTrackParam::CovMatrix(Double_t cov[]) const {
...
void CbmTrackParam::CovMatrix(TMatrixFSym& covMat) const {
  Int_t index = 0;
  for (int i=0; i<5; i++) {
    for (int j=i; j<5; j++) {
...
Double_t CbmTrackParam::GetCovariance(Int_t i, Int_t j) const {
  if ( i<0 || j<0 || i>4 || j>4 ) {
    cout << "-E- CbmTrackParam::GetCovariance: Invalid index pair ("
	 << i << "," << j << ") !" << endl;
    return 0;
  }
  if (i>j) {
...
// -----   Position modifier    --------------------------------------------
void CbmTrackParam::SetPosition(const TVector3& pos) {
  fX = pos.X();
  fY = pos.Y();
  fZ = pos.Z();
}
...
void CbmTrackParam::SetCovMatrix(Double_t cov[]) {
...
void CbmTrackParam::SetCovMatrix(const TMatrixFSym& covMat) {
  Int_t index = 0;
  for (int i=0; i<5; i++) {
    for (int j=i; j<5; j++) fCovMatrix[index++] = covMat[i][j];
  }
...
void CbmTrackParam::SetCovariance(Int_t i, Int_t j, Double_t val) {
  if (i < 0 || i > 4) {
    cout << "-W- CbmTrackParam::SetCovariance: " 
...
// -----   Assignment operator   -------------------------------------------
CbmTrackParam& CbmTrackParam::operator=(const CbmTrackParam& par){
  fX  = par.GetX();
  fY  = par.GetY();
  fZ  = par.GetZ();
  fTx = par.GetTx();
