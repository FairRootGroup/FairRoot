...
class TParticle;
class TGeoTrack;
class CbmVTrack : public TPolyLine3D {
public:
  CbmVTrack();
  CbmVTrack(const CbmVTrack& pts);
  CbmVTrack(Int_t nhits);
  virtual ~CbmVTrack();
  ...
  CbmVTrack &           operator=(const CbmVTrack &pts)
...
protected:
  ...
  Int_t          fIndex;      
  TParticle     *fParticle;
  ClassDef(CbmVTrack,1) //Class to  Tracks (Also in OpenGL viewer of ROOT)
};
#endif
