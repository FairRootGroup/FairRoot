...
InitStatus FairPrintFairLinks::Init()
{
    ...
    if (!ioman) {
        LOG(error) << "-E- FairPrintFairLinks::Init: " << "RootManager not instantiated!";
        return kFATAL;
    }
...
void FairPrintFairLinks::Exec(Option_t*)
{
    ...
    for (std::map<Int_t, TClonesArray*>::iterator iter = fBranches.begin(); iter != fBranches.end(); iter++){
        LOG(info) << "\n" << iter->first << " : " << FairRootManager::Instance()->GetBranchName(iter->first) << " Entries: " << iter->second->GetEntriesFast();
