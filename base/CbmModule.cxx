...
TArrayI* CbmModule::volNumber=0;
Int_t CbmModule::fNbOfVolumes=0;
CbmVolumeList*  CbmModule::vList=0;
TRefArray*  	CbmModule::svList=0;
...
//__________________________________________________________________________
CbmModule::CbmModule(const char * Name, const char *title ,Bool_t Active)
	:TNamed(Name, title),
	 fgeoVer("Not defined"),
...
void CbmModule::Streamer(TBuffer& b)
{
  TNamed::Streamer(b);
  if (b.IsReading()) {
    fgeoVer.Streamer(b);
...
void CbmModule::SetGeometryFileName(TString fname, TString geoVer)
{
   fgeoVer=geoVer; 
...
void CbmModule::ProcessNodes(TList *aList){

  TListIter iter(aList);
  CbmGeoNode* node   = NULL;
  CbmGeoNode* MotherNode =NULL;
  CbmVolume*  volume = NULL;
 ...
 //   GetListOfGeoPar()->Add( node );
    ...
    volume = new CbmVolume( node->getTruncName(), fNbOfVolumes++);
...
/*  cout << " CbmModule::ProcessNodes "<< endl;
	 svList->ls();
  cout << " CbmModule::ProcessNodes "<< endl;
*/
}

void	CbmModule::AddSensitiveVolume(TGeoVolume *v){

//	cout <<"CbmModule::AddSensitiveVolume  " << v->GetName() << endl;
  	CbmVolume*  volume = NULL;
  	volume = new CbmVolume(v->GetName(), fNbOfVolumes++);
  	vList->addVolume(volume);
  	volume->setModId(fModId);
  	volume->SetModule(this);
  	svList->Add(volume);
...
CbmVolume* CbmModule::getCbmVolume(CbmGeoNode *fN)
{
	CbmVolume *fv;
	CbmVolume  *fvol=0;
//	cout << " CbmModule::getCbmVolume " << fN <<  endl;
...
void CbmModule::ConstructRootGeometry(){
   ...
   if(v1==0) {
	   cout << "CbmModule::ConstructRootGeometry(): could not find any geometry in File!! " << GetGeometryFileName().Data() <<  endl; 
	   exit(0);
   }
   gGeoManager=OldGeo;
   gGeoManager->cd();
...
void CbmModule::ConstructASCIIGeometry(){
	cout << " CbmModule::ConstructASCIIGeometry() : this method has to be implimented in detector class " << endl;
}
Bool_t CbmModule::CheckIfSensitive(std::string name){
	cout << " CbmModule::CheckIfSensitive(std::string name): this method has to be implimented in detector class " << endl;
	return kFALSE;
}
void CbmModule::ExpandNode(TGeoNode *fN){
   ...
   for (Int_t Nod=0; Nod<NodeList->GetEntriesFast();Nod++) {   
      ...
      if(med1){
        ...
        std::cout<<" -E- Error in CbmModule::ExpandNode()!  "
