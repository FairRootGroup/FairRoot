...
//        decayer->ForceParticleDecay(443,products,mult,npart);
	...
	//     TVirtual::GetMC()->SetUserDecay(pdgId);
	...
	TVirtual::GetMC()->SetExternalDecayer(decayer);
	...
	for ( Int_t ipartnf = 0; ipartnf < npartnf; ipartnf++ ) {
	 Int_t ipdg = pdgnf[ipartnf];
        ...
        if (TString(TVirtual::GetMC()->GetName()) == "TGeant3") TVirtual::GetMC()->SetUserDecay(ipdg);// Force the decay to be done w/external decayer
