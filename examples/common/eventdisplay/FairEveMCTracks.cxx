...
FairEveMCTracks::FairEveMCTracks():FairEveTracks(),
...
Bool_t FairEveMCTracks::CheckCuts(FairMCTrack *tr) {
    ...
    tr->Get4Momentum(mom);
    if(UsePtCut()){
        if(tr->GetPt()<GetPtMin()) return kFALSE;
        if(tr->GetPt()>GetPtMax()) return kFALSE;
    }
    if(UseEtaCut()){
    ...
    if(UseEnergyCut()){
        if(mom.Energy()<GetEnergyMin()) return kFALSE;
        if(mom.Energy()>GetEnergyMax()) return kFALSE;
    }
    if(fUsePdg){
        if(tr->GetPdgCode()!=fPdgCut) return kFALSE;
    }
    Bool_t primary = kTRUE;
    if(tr->GetMotherId()!=-1)
        primary = kFALSE;
    if(fShowPrimary&&primary) return kTRUE;
    if(fShowSecondary&&!primary) return kTRUE;
    return kFALSE;
}
...
void FairEveMCTracks::DrawTrack(Int_t id) {
    ...
    TParticle p(tr->GetPdgCode(),0,0,0,0,0,tr->GetPx(),tr->GetPy(),tr->GetPz(),
            tr->GetEnergy(),tr->GetStartX(),tr->GetStartY(),tr->GetStartZ(),tr->GetStartT());
