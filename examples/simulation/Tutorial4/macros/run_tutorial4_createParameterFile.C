...
void run_tutorial4_createParameterFile()
{
    ...
    for (Int_t iNode = 0; iNode < nodes->GetEntriesFast(); iNode++) {
        ...
        if (!TString(node->GetName()).Contains("tutorial4"))
        ...
        for (Int_t iLayer = 0; iLayer < layers->GetEntriesFast(); iLayer++) {
            ...
            if (misalignInfo.find(hash) != misalignInfo.end()) {
                ...
                cerr << "[ERROR ] TGeoNode: " << it->second.first << " and " << path << " both have hash " << hash
                     << endl;
    ...
    for (auto& elem : misalignInfo) {
        cout << "Module " << elem.first << " " << elem.second.first << " " << elem.second.second << endl;
    }
...
*/
    ...
    for (auto& elem : misalignInfo) {
        const Double_t* rotation = elem.second.second->GetRotationMatrix();
        const Double_t* translation = elem.second.second->GetTranslation();
        myfile << "Module " << elem.first << " " << elem.second.first << " " << rotation[0] << " " << rotation[1] << " "
               << rotation[2] << " " << rotation[3] << " " << rotation[4] << " " << rotation[5] << " " << rotation[6]
               << " " << rotation[7] << " " << rotation[8] << " " << translation[0] << " " << translation[1] << " "
               << translation[2] << endl;
    }
    myfile.close();
}
