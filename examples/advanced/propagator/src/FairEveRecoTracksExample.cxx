...
FairEveRecoTracksExample::FairEveRecoTracksExample():FairEveTracks(),
fContainerReco(nullptr),fContainerSim(nullptr),fHits1(nullptr),
fHits2(nullptr),
fShowPrimary(kTRUE),fShowSecondary(kTRUE),
fDrawMC(kTRUE),fUsePdg(kFALSE),
fPdgCut(0) ,fRK(nullptr),fPDG(nullptr) {
...
Bool_t FairEveRecoTracksExample::CheckCuts(FairTutPropTrack *tr) {
    ...
    if(UsePtCut()){
        if(mom.Pt()<GetPtMin()) return kFALSE;
        if(mom.Pt()>GetPtMax()) return kFALSE;
    }
    if(UseEtaCut()){
        if(mom.Eta()<GetEtaMin()) return kFALSE;
        if(mom.Eta()>GetEtaMax()) return kFALSE;
    }
    //assume pion mass
    Double_t energy = TMath::Sqrt(mom.Mag2()+0.139*0.139);
    if(UseEnergyCut()){
        if(energy<GetEnergyMin()) return kFALSE;
        if(energy>GetEnergyMax()) return kFALSE;
    }
    return kTRUE;
}
...
void FairEveRecoTracksExample::DrawTrack(Int_t id) {
    ...
    TParticle p(dummy_pid,0,0,0,0,0,par.GetPx(),par.GetPy(),par.GetPz(),
            0,par.GetX(),par.GetY(),par.GetZ(),0);
    ...
    Double_t vecRKIn[7]={
            pos.X(),pos.Y(),pos.Z(),
            mom.Px()*P,mom.Py()*P,mom.Pz()*P,1.0/P};
    ...
    if(fDrawMC&&tr->GetMCTrackIndex()>=0){
        ...
        TParticle p_mc(mc->GetPdgCode(),0,0,0,0,0,mc->GetPx(),mc->GetPy(),mc->GetPz(),
                    mc->GetEnergy(), mc->GetStartX(),mc->GetStartY(),mc->GetStartZ(),mc->GetStartT());
...
void FairEveRecoTracksExample::SwapTracks() {
    for (int i = 0; i < GetTracksList()->GetEntriesFast(); i++) {
        ...
        for (TEveElement::List_i iter = eveList->BeginChildren();
                iter != eveList->EndChildren(); ++iter) {
