...
// -----   Default constructor   -------------------------------------------
CbmGeanePro::CbmGeanePro() : TNamed("Geane", "Propagate Tracks") 
{ 
  gMC3 = (TGeant3*) gMC;        
  if(gMC3==NULL){
    std::cerr<<"CbmGeanePro::TGeant3 has not been initialized! ABORTING!"<<std::endl;
    throw;
  }
  nepred=1;
  ...
  Mom=TVector3(0,0,0);
  fTrkPar= new CbmTrackPar();
  ProMode=0;
  CbmRunAna *fRun= CbmRunAna::Instance();
  fField= fRun->GetField();
  fPCA = 0;
  fApp =CbmMCApplication::Instance();
}

// -----   Destructor   ----------------------------------------------------
CbmGeanePro::~CbmGeanePro() { }



Bool_t CbmGeanePro::Propagate(CbmTrackParH *TParam, CbmTrackParH *TEnd, Int_t PDG)
{
...
Bool_t CbmGeanePro::Propagate(CbmTrackParP *TStart, CbmTrackParH *TEnd, Int_t PDG)
{
  // Propagate a parabola track (SD system) and return a helix (SC system) (not used nor implemented)
  cout << "CbmGeanePro::Propagate(CbmTrackParP *TParam, CbmTrackParH &TEnd, Int_t PDG) : (not used nor implemented)" << endl;
  return kFALSE;
}

Bool_t CbmGeanePro::Propagate(CbmTrackParP *TStart, CbmTrackParP *TEnd, Int_t PDG)
{
  ...
  if (ProMode==1){ //Propagate to Volume  
	...
	// output 
	Int_t findpca = FindPCA(fPCA, PDG, fpoint, fwire1, fwire2, maxdistance, Rad, vpf, vwi, Di, trklength);
	//std::cout<<" CbmGeanePro::trklength="<<trklength<< std::endl;
	if(findpca != 0) return kFALSE;
...
Bool_t CbmGeanePro::Propagate(CbmTrackParH *TStart, CbmTrackParP *TEnd, Int_t PDG)
{
  // Propagate a helix track (SC system) and return a parabola (SD system) (not used nor implemented)
  cout << "CbmGeanePro::Propagate(CbmTrackParH *TParam, CbmTrackParP &TEnd, Int_t PDG)" << endl;
  return kTRUE;  
}
...
void CbmGeanePro::Propagate(Int_t PDG) {
  ...
  GeantCode=fdbPDG->ConvertPdgToGeant3(PDG);
  cout <<  " CbmGeanePro::Propagate ---------------------------"<< "  " << x1[0]<< " "<< x1[1]<< "  "<<  x1[2] << endl; 
  fApp->GeanePreTrack(x1, p1, PDG);
...
Bool_t CbmGeanePro::PropagateFromPlane(TVector3 &v1, TVector3 &v2)
{
...
Bool_t CbmGeanePro::PropagateToPlane(TVector3 &v0, TVector3 &v1, TVector3 &v2)
{
...
Bool_t CbmGeanePro::PropagateToVolume(TString VolName, Int_t CopyNo , Int_t option)
{
...
Bool_t CbmGeanePro::PropagateToLength(Float_t length)
{
...
Bool_t CbmGeanePro::SetWire(TVector3 extremity1, TVector3 extremity2)
{
...
Bool_t CbmGeanePro::SetPoint(TVector3 pnt)
{
...
Bool_t CbmGeanePro::PropagateToPCA(Int_t pca)
{
...
Bool_t CbmGeanePro::ActualFindPCA(Int_t pca, CbmTrackParP *par, Int_t dir)
{
  Init(par);
  // through track length
  if(dir > 0) fPropOption="LE";
  else if(dir < 0) fPropOption="BLE";
  else cout << "CbmGeanePro::ActualFindPCA ERROR: no direction set" << endl;
  ProMode=1; //need errors in representation 1 (SC)(see Geane doc)   
  fPCA = pca;
  // initialization 
  Rad = 0.;
...
Bool_t CbmGeanePro::BackTrackToVertex()
{
  // through track length
  fPropOption="BLE";
  ProMode=1; //need errors in representation 1 (SC)(see Geane doc)   
...
Bool_t CbmGeanePro::PropagateToVirtualPlaneAtPCA(Int_t pca)
{
  // through track length
  fPropOption="LE";
  ProMode=3; //need errors in representation 3 (SD)(see Geane doc) 
...
Bool_t CbmGeanePro::BackTrackToVirtualPlaneAtPCA(Int_t pca)
{
  // through track length
  fPropOption="BLE";
  ProMode=3; //need errors in representation 3 (SD)(see Geane doc) 
...
int CbmGeanePro::FindPCA(Int_t pca, Int_t PDGCode, TVector3 point, TVector3 wire1, TVector3 wire2, Double_t maxdistance, Double_t &Rad, TVector3 &vpf, TVector3 &vwi, Double_t &Di, Float_t &trklength)
{
...
void CbmGeanePro::Track2ToLine( TVector3 x1,  TVector3 x2,  TVector3 w1,
...
void CbmGeanePro::Track2ToPoint( TVector3 x1,  TVector3 x2,  TVector3 w1, TVector3 &Pfinal, 
...
void CbmGeanePro::Track3ToLine(TVector3 x1, TVector3 x2, TVector3 x3, 
...
void CbmGeanePro::Track3ToPoint( TVector3 x1, TVector3 x2, TVector3 x3, TVector3 w1, 
				 // output
...
void CbmGeanePro::GetTransportMatrix(Double_t trm[5][5])
{
