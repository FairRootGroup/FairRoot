...
FairEveAnimationButton::FairEveAnimationButton(TGedFrame *frame,TGCompositeFrame *tab,TString name,Int_t width,Int_t buttons):
fWidth(width),fParent(frame),fTab(tab),fMin(nullptr),
fMax(nullptr),fStep(nullptr),fComboOpt(nullptr),fValMin(0),fValMax(10),fTypeOpt(nullptr){
    SetName(name);
}
...
Double_t FairEveAnimationButton::GetMin() {
    return fMin->GetNumber();
}

Double_t FairEveAnimationButton::GetMax() {
    return fMax->GetNumber();
}
