...
void FairEventManagerEditor::Init()
{
    ...
    fCurrentEvent = new TGNumberEntry(f, 0., 6, -1, TGNumberFormat::kNESInteger,
    ...
    title1->AddFrame(fUpdate,
            new TGLayoutHints(kLHintsRight | kLHintsExpandX, 5, 5, 1, 1));
    fUpdate->Connect("Clicked()", "FairEventManagerEditor", this,
            "SelectEvent()");
    ...
    TGCheckButton *Transparency = new TGCheckButton(transparency_frame,
            "Global transparency");
    transparency_frame->AddFrame(Transparency,
            new TGLayoutHints(kLHintsLeft | kLHintsExpandX, 5, 5, 1, 1));
    Transparency->Connect("Toggled(Bool_t)", this->ClassName(), this,
            "SwitchTransparency(Bool_t)");

    fGlobalTransparency = new TGNumberEntry(transparency_frame, 0., 6, -1,
            TGNumberFormat::kNESInteger, TGNumberFormat::kNEANonNegative,
            TGNumberFormat::kNELLimitMinMax, 0, 100);
    fGlobalTransparency->SetIntNumber(80);
    transparency_frame->AddFrame(fGlobalTransparency,
            new TGLayoutHints(kLHintsRight, 1, 1, 1, 1));
    fGlobalTransparency->Connect("ValueSet(Long_t)", "FairEventManagerEditor",
            this, "SwitchTransparency(Bool_t)");
    scene_conf->AddFrame(transparency_frame);

    TGCheckButton *backgroundButton = new TGCheckButton(scene_conf,
            "Light background");
    scene_conf->AddFrame(backgroundButton,
            new TGLayoutHints(kLHintsRight | kLHintsExpandX, 5, 5, 1, 1));
    backgroundButton->Connect("Toggled(Bool_t)", this->ClassName(), this,
    ...
    frame_screenshot->AddFrame(fScreenshotOpt,
            new TGLayoutHints(kLHintsRight | kLHintsExpandX));
    scene_conf->AddFrame(frame_screenshot,
            new TGLayoutHints(kLHintsTop | kLHintsExpandX, 1, 1, 2, 1));

    fAnimation = new FairEveAnimationButton(this, scene_conf, "Animation",
            fWidth, 2);
