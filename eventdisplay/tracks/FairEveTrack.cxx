...
FairEveTrack::FairEveTrack() :TEveTrack(),fNPoints(0){
}

FairEveTrack::FairEveTrack(TParticle *t, Int_t label,
        TEveTrackPropagator *prop) :TEveTrack(t,label,prop),fNPoints(0){
}
...
void FairEveTrack::SetNextPoint(const TVector3 &point) {
    if(TMath::Abs(point.X())>fgWorldSize) return;
    if(TMath::Abs(point.Y())>fgWorldSize) return;
    if(TMath::Abs(point.Z())>fgWorldSize) return;
    if(TMath::IsNaN(point.X())) return;
