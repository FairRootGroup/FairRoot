...
fWidth(width),fName(name),
fParent(frame),fTab(tab) {
}

void FairEveCut::AddUpdateButton() {
    TGCompositeFrame *Frame = new TGCompositeFrame(fTab, fWidth, 20,
            kHorizontalFrame | kFixedWidth);
    TGTextButton *UpdateButton = new TGTextButton(Frame, "Update");
    UpdateButton->Connect("Clicked()", fParent->ClassName(), fParent,
            "Repaint()");
...
FairEveMinMaxCut::FairEveMinMaxCut(TGedFrame *frame, TGCompositeFrame *tab,  TString name,Int_t width):
    FairEveCut(frame,tab,name,width),fUseCut(nullptr),fLow(nullptr),fHigh(nullptr){
}
...
FairEveBoolCut::FairEveBoolCut(TGedFrame *frame, TGCompositeFrame *tab,TString name,Int_t width) :
        FairEveCut(frame,tab,name,width),fHasCut(nullptr),fStatus(kTRUE),fAutoUpdate(kFALSE){
}

void FairEveBoolCut::Init() {
   fHasCut = new TGCheckButton(fTab,fName,1);
   if(fAutoUpdate){
       fHasCut->Connect("Clicked()", fParent->ClassName(), fParent,
               "Repaint()");
   }
   if(fStatus){
...
FairEveIntCut::FairEveIntCut(TGedFrame *frame, TGCompositeFrame *tab,
        TString name, Int_t width) :FairEveCut(frame,tab,name,width),fUseCut(nullptr),fCut(nullptr){
}
