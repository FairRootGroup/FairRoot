...
FairEveGeoTracks::FairEveGeoTracks():FairEveTracks(),
fContainer(nullptr),fShowPrimary(kTRUE),fShowSecondary(kTRUE),fUsePdg(kFALSE),
fPdgCut(0),fTMin(0),fTMax(std::numeric_limits<Double_t>::max()){
...
void FairEveGeoTracks::DrawTrack(Int_t id) {
    ...
    track->SetElementTitle(
            Form("p={%4.3f,%4.3f,%4.3f}", p->Px(), p->Py(), p->Pz()));
...
void FairEveGeoTracks::DrawAnimatedTrack(Int_t id) {
    ...
    TGeoTrack *tr = (TGeoTrack*)fContainer->UncheckedAt(id);
    if(!CheckCuts(tr))return;
    if(tr->GetNpoints()<2) return;//not enought points
    if(tr->GetPoint(0)[3]*timeScale<fTMin) return;//first point before tmin
    if(tr->GetPoint(0)[3]*timeScale>fTMax)return;//first point after tmax
    TParticle *p = (TParticle*) tr->GetParticle();
    TEveTrackList *trList = GetTrackGroup(tr);
    Color_t color = GetEventManager()->Color(p->GetPdgCode());
    FairEveTrack *track = new FairEveTrack(p, p->GetPdgCode(), fTrPropagator);
...
Bool_t FairEveGeoTracks::CheckCuts(TGeoTrack *tr) {
    TParticle *p = (TParticle*)tr->GetParticle();
    if(UsePtCut()){
        if(p->Pt()<GetPtMin()) return kFALSE;
        if(p->Pt()>GetPtMax()) return kFALSE;
    }
    if(UseEtaCut()){
        if(p->Eta()<GetEtaMin()) return kFALSE;
        if(p->Eta()>GetEtaMax()) return kFALSE;
    }
    if(UseEnergyCut()){
        if(p->Energy()<GetEnergyMin()) return kFALSE;
        if(p->Energy()>GetEnergyMax()) return kFALSE;
    }
    if(fUsePdg){
        if(p->GetPdgCode()!=fPdgCut) return kFALSE;
    }
    if(fShowPrimary&&p->IsPrimary()) return kTRUE;
    if(fShowSecondary&&!p->IsPrimary()) return kTRUE;
    return kFALSE;
}
