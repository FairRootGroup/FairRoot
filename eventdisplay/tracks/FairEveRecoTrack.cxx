...
TEveCompound(),fHits(nullptr),fNPoints(0),fRecoTrack(nullptr),fMCTrack(nullptr){
}

FairEveRecoTrack::FairEveRecoTrack(TParticle *t, Int_t label,
        TEveTrackPropagator *prop) :TEveCompound(),fHits(nullptr),fNPoints(0),
                fRecoTrack(new FairEveTrack(t,label,prop)),fMCTrack(nullptr){
    OpenCompound();
    AddElement(fRecoTrack);
}

void FairEveRecoTrack::AddHit(TVector3 hit) {
    if(TMath::Abs(hit.X())>FairEveTrack::GetWorldSize()) return;
    if(TMath::Abs(hit.Y())>FairEveTrack::GetWorldSize()) return;
    if(TMath::Abs(hit.Z())>FairEveTrack::GetWorldSize()) return;
    if(fHits==nullptr){
...
void FairEveRecoTrack::MakeMCTrack(const TGeoTrack *geo) {
    if(fMCTrack!=nullptr) return;
    TParticle *p = static_cast<TParticle*>(geo->GetParticle());
    fMCTrack = new FairEveTrack(p,
            fRecoTrack->GetLabel(),fRecoTrack->GetPropagator());
...
void FairEveRecoTrack::SetMainColor(Color_t color) {
    ...
    if(fRecoTrack) fRecoTrack->SetMainColor(color);
    if(fHits) fHits->SetMainColor(color);
    if(fMCTrack) fMCTrack->SetMainColor(color);
}
