...
#include "FairTimebasedMCPointDraw.h"
...
FairTimebasedMCPointDraw::FairTimebasedMCPointDraw()
{
...
InitStatus FairTimebasedMCPointDraw::Init()
{
    ...
    if (fEventTime == nullptr) {
        LOG(error) << "FairTimebasedMCPointDraw::Init no EventTimes vector";
        return kERROR;
    }

    LOG(debug2) << "FairTimebasedMCPointDraw::Init() get track list " << fPointList;

    FairPointSetDraw::Init();
    return kSUCCESS;
}

void FairTimebasedMCPointDraw::GetData()
{
    ...
    if (evtIndex > -1) {
        LOG(debug) << "FairTimebasedMCPointDraw::GetData " << simTime << " lower " << *lower << " at index "
                   << evtIndex + 1;
...
Int_t FairTimebasedMCPointDraw::GetNPoints() { return fDataHandler.GetData().size(); }

TVector3 FairTimebasedMCPointDraw::GetVector(Int_t index)
{
    FairMCPoint* p = fDataHandler.GetData()[index].first;
    return TVector3(p->GetX(), p->GetY(), p->GetZ());
}

Double_t FairTimebasedMCPointDraw::GetTime(Int_t index)
{
    return fDataHandler.GetData()[index].first->GetTime()
           + fDataHandler.GetData()[index].second;   // Flight time + event time
}

ClassImp(FairTimebasedMCPointDraw);
