...
ClassImp(CbmGeoOldAsciiIo)

CbmGeoOldAsciiIo::CbmGeoOldAsciiIo() 
  : filename(""),
    filedir(""),
    writable(kFALSE),
	file(0)
...
Bool_t CbmGeoOldAsciiIo::read(CbmGeoSet* set,CbmGeoMedia* media) {
  ...
  CbmGeoShapes* pShapes=set->getShapes();
  while(!fin.eof()) {
    // Read volumeName
    TString volumeName = "";
    fin >> volumeName;
...
// Why this additional integer only in trd file ????????????
    ...
    CbmGeoTransform& tf=volu->getTransform();
    tf.setRotMatrix(r);
    tf.setTransVector(t);
    // Read material
    TString materialName = "";
    fin >> materialName;
    CbmGeoMedium* medium=media->getMedium(materialName);
    if (!medium) {
      medium=new CbmGeoMedium(materialName);
    ...
    TString type = "";
    fin >> type;
    CbmGeoBasicShape* sh=pShapes->selectShape(type);
...
Bool_t CbmGeoOldAsciiIo::calculateShapePoints(Double_t* par,CbmGeoNode* volu) {
