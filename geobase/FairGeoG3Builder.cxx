...
ClassImp(FairG3MapedNames)
ClassImp(FairGeoG3Builder)

TMap FairG3MapedNames::fVolumeMap;
TMap FairG3MapedNames::fMediumMap;

void FairG3MapedNames::saveVolumeName( const char* tname, const char* vname){
...
void FairG3MapedNames::saveMediumName( const char* tname, const char* vname){
...
const char* FairG3MapedNames::getTruncVolName( const char *vname){
    TNamed *fObj=NULL;
...
const char* FairG3MapedNames::getTruncMedName( const char *vname){
...
void  FairG3MapedNames:: listVolumes() {
    TNamed* fKey=NULL;
    TNamed* fVal=NULL;
...
void  FairG3MapedNames:: listMedium() {
    TNamed* fKey=NULL;
    TNamed* fVal=NULL;
...
const char*  FairG3MapedNames::getVolRealName(const char* tname) {
    TNamed* fKey=NULL;
    TNamed* fVal=NULL;
    TMapIter next ( &fVolumeMap);
    while ( (fKey = dynamic_cast<TNamed*> (next())) ){
...
const char*  FairG3MapedNames::getMedRealName(const char* tname) {
    TNamed* fKey=NULL;
    TNamed* fVal=NULL;
    TMapIter next ( &fMediumMap);
    while ( (fKey = dynamic_cast<TNamed*> (next())) ){
...
FairGeoG3Builder::FairGeoG3Builder() {
  // Default constructor
  gMC3 = (TGeant3*) gMC;
  fDebug=kTRUE;
  fCountVolume=0;
  fCountMedium=0;
  fMapedNames = new FairG3MapedNames();
}

FairGeoG3Builder::FairGeoG3Builder(const char* name,const char* title)
...
Bool_t FairGeoG3Builder::createNode(FairGeoNode* volu, Int_t hadFormat) {
...
Int_t FairGeoG3Builder::createMedium(FairGeoMedium* med) {
   if (!gMC3 ) gMC3 = (TGeant3*) gMC;
  fCountMedium++;
...
const char* FairGeoG3Builder::truncateName( Int_t id ){
  ...
  */
  ...
  if (fDebug) cout << " -I-  FairGeoG3Builder G3 truncated name created " << tName << endl;
  return tName;
}
